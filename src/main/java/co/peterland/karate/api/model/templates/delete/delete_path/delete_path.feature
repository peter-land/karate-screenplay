Feature: delete path
  ...
  Scenario: send
    ...
    When method delete
#    Then status 200
    * call read('classpath:co/peterland/karate/api/model/templates/print/print_request/print_request.feature') { baseUrl: '#(baseUrl)', path: '#(path)', operation: '#(operation)', body: '#(body)', response: '#(response)'}
